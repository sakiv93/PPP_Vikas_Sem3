...
functions. !The pioneers of this technique are Tom Hughes and his group at The
...
\begin{description}[leftmargin=*]
	...
	\item[$\bullet$]   A huge amount of time involved in finite element modelling
	can be avoided.
\end{description}
...
the control points affect the NURBS curve. The number of knots is always equal
to the number of control points plus curve degree plus one (i.e. number of
control points plus curve order). The knot vector divides the parametric space
in the intervals mentioned before, usually referred to as knot spans. Each time
the parameter value enters a new knot span, a new control point becomes active,
while an old control point is discarded. It follows that the values in the knot
vector should be in nondecreasing order, so (0, 0, 1, 2, 3, 3) is valid while
(0, 0, 2, 1, 3, 3) is not.(WIKI)!

\subsection{Control points }
!The control points determine the shape of the curve.[8] Typically, each point
of the curve is computed by taking a weighted sum of a number of control points.
The weight of each point varies according to the governing parameter. For a
curve of degree d, the weight of any control point is only nonzero in d+1
intervals of the parameter space. Within those intervals, the weight changes
according to a polynomial function (basis functions) of degree d. At the
boundaries of the intervals, the basis functions go smoothly to zero, the
smoothness being determined by the degree of the polynomial.(WIKI)! The total
...
\begin{enumerate}[leftmargin=*]
	...
	Fig.(). The NURBS basis functions and their derivatives are evaluated at
...
displacement and stress fields are plotted. 
...
\begin{figure}[H]
	\begin{center}
		...
		\caption{\\Flow chart describing Pre-processing, Processing and
			Post-processing Stage of IGA}\label{FlowChart1}
	\end{center}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{XYLoading.png}
...
Abaqus plane strain full integration piezoelectric element (\textbf{CPE4}) is used for analysis. The below figures shows the values of displacements (U), electrical potentials (EPOT) and reaction forces (RF ) for both abaqus and IGA element.\\
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{CPE4 Element:U1}
		\label{M1U1}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{M1U1_IGA.png}
		\captionof{figure}{IGA Element:U1}
		\label{M1U1_IGA}
	\end{minipage}
\end{figure}
\begin{comment}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{IGA Element:U2}
		\label{M1U2_IGA}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{IGA Element:RF1}
		\label{M1RF1_IGA}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{IGA Element:RF2}
		\label{M1RF2_IGA}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{IGA Piezoelectric Element:RF1}
		\label{EM1RF1_IGA}
	\end{minipage}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{IGA Piezoelectric Element:RF2}
		\label{EM1RF2_IGA}
	\end{minipage}
\end{figure}
